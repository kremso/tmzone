...
describe Indprop::MarkParser do
  context 'when parsing texual mark' do
    it 'parses mark attributes' do
      ...
      subject.parse(File.read('spec/fixtures/textual_mark.htm'), mark)
    end
  end
  ...
  context 'when parsing visual mark' do
    it 'parses mark attributes' do
      ...
      subject.parse(File.read('spec/fixtures/visual_mark.htm'), mark)
    end
  end

  it 'returns the mark' do
      mark = double(:Hit).as_null_object
      subject.parse(File.read('spec/fixtures/visual_mark.htm'), mark).should == mark
  end
