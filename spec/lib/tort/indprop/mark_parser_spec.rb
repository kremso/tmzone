...
describe Tort::Indprop::MarkParser do
  context 'when parsing texual mark' do
    it 'parses mark attributes' do
      ...
      mark.should_receive(:application_date=).with(Date.parse("18.10.2002"))
      mark.should_receive(:registration_date=).with(Date.parse("05.03.2003"))
      ...
      mark.should_receive(:valid_until=).with(Date.parse("18.10.2012"))
  ...
  context 'when parsing visual mark' do
    it 'parses mark attributes' do
      ...
      mark.should_receive(:application_date=).with(Date.parse("13.03.2007"))
      mark.should_receive(:registration_date=).with(Date.parse("13.09.2007"))
      ...
      mark.should_receive(:valid_until=).with(Date.parse("13.03.2017"))
      mark.should_receive(:status=).with('Platná')
