...
describe Tort::CTM::MarkParser do
  context 'when parsing texual mark' do
    it 'parses mark attributes' do
      ...
      mark.should_receive(:application_date=).with(Date.parse("01.04.1996"))
      mark.should_receive(:registration_date=).with(Date.parse("28.10.1998"))
      ...
      mark.should_receive(:valid_until=).with(Date.parse("01.04.2016"))
  ...
  context 'when parsing visual mark' do
    it 'parses mark attributes' do
      ...
      mark.should_receive(:application_date=).with(Date.parse("01.04.1996"))
      mark.should_receive(:registration_date=).with(Date.parse("25.05.1998"))
      ...
      mark.should_receive(:valid_until=).with(Date.parse("01.04.2006"))
